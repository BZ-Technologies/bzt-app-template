# {App Name} Application Deployment

## Overview

This document describes how to deploy the {App Name} application to tenant infrastructure as part of the BZ Technologies platform.
...
```bash
cd /opt
git clone git@github.com:BZ-Technologies/bzt-app-{app-code}.git
cd bzt-app-{app-code}
```
...
```javascript
const {appCode}Routes = require('/opt/bzt-app-{app-code}/src/routes/{appCode}Routes');
...
```bash
cp -r /opt/bzt-app-{app-code}/client/src/app/{route} /opt/bzt-portal/client/src/app/
```
...
*/5 * * * * cd /opt/bzt-app-{app-code} && /usr/bin/node src/workers/{appCode}Worker.js >> /var/log/{app-code}-worker.log 2>&1
```

#### Option B: PM2

```bash
pm2 start src/workers/{appCode}Worker.js --name {app-code}-worker --cron "*/5 * * * *"
pm2 save
```
