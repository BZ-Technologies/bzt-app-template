#!/bin/bash
# Deployment script for {App Name} application
...
APP_NAME="{App Name}"
APP_CODE="{app-code}"
REPO_PATH="/opt/bzt-app-{app-code}"
...
if [ -f "$REPO_PATH/scripts/run-migrations.js" ]; then
    ...
    if command -v mysql &> /dev/null; then
        ...
        echo "   Running: db-init-{app-code}.sql"
        mysql -u root -p "$DB_NAME" < "$REPO_PATH/migrations/db-init-{app-code}.sql"
        
        echo "   Running: db-add-{app-code}-app.sql"
        mysql -u root -p "$DB_NAME" < "$REPO_PATH/migrations/db-add-{app-code}-app.sql"
    else
